    ...
    , ui(new Ui::QIPMSG)
{
    ...
    connect(&session,
            ...
            this,
            SLOT(qIpMsgUdpSessionHandle(QHostAddress, QByteArray)));
    connect(fileServer,
            SIGNAL(ipMsgFileServerDataReady(quint32, quint16, QByteArray)),
            this,
            SLOT(qIpMsgFileServerHandle(quint32, quint16, QByteArray)));
    connect(this, SIGNAL(userListUpdate()), this, SLOT(qIpMsgUserListUpdate()));
    connect(ui->listWidgetUserList,
            SIGNAL(itemDoubleClicked(QListWidgetItem *)),
            this,
            SLOT(startChat(QListWidgetItem *)));
...
void QIPMSG::qIpMsgSettingsAccepted()
{
    ...
    qDebug() << "Username:" << mSelf.userId << "NickName:" << mSelf.userNickName
             << "GroupName:" << mSelf.userGroupName;
    qDebug() << "StartIP:" << QHostAddress(mStartIp).toString()
...
void QIPMSG::userListRefreshTrigger()
{
    ...
    if (true == session.IpMsgUdpSessionValid()) {
        for (int i = 0; i < mUsers.count(); i++) {
            ...
            if ((mUsers.at(i)->chatForm == nullptr || mUsers.at(i)->chatForm->isHidden())) {
                if (mUsers.at(i)->chatForm != nullptr) {
...
void QIPMSG::qIpMsgUserListUpdate()
{
    ...
    for (int i = 0; i < count; i++) {
        ui->listWidgetUserList->addItem(
            new QListWidgetItem(QIcon(":/userIcon/chat.png"),
                                toUnicode(mUsers.at(i)->userNickName) + "("
...
void QIPMSG::startChat(QListWidgetItem *item)
{
    ...
    ui->listWidgetUserList->item(currentRow)->setForeground(QBrush(QColor("Black")));
...
void QIPMSG::timerEvent(QTimerEvent *event)
{
    ...
    for (int i = 0; i < mUsers.count(); i++) {
        ...
        if (mUsers.at(i)->offlineTimer != 0 && mUsers.at(i)->offlineTimer == current_timer) {
            killTimer(mUsers.at(i)->offlineTimer);
            mUsers.at(i)->offlineTimer = 0;
            if (mUsers.at(i)->userCfmSeq != mUsers.at(i)->userPktSeq) {
                mUsers.at(i)->appendChatHistory(
                    toUnicode(mUsers.at(i)->userNickName + tr(" May be offline").toUtf8() + "\n"));
                mUsers.at(i)->updateChatFormHistory();
            }
            break;
        }
        //New message arrived, blink user icon
        else if (mUsers.at(i)->blinkTimer != 0 && mUsers.at(i)->blinkTimer == current_timer) {
            if (mUsers.at(i)->blinkFlag % 2) {
                ui->listWidgetUserList->item(i)->setIcon(QIcon(":/userIcon/ipmsg.png"));
                ui->listWidgetUserList->item(i)->setForeground(QBrush(QColor("Red")));
            } else {
                ui->listWidgetUserList->item(i)->setIcon(QIcon(":/userIcon/chat.png"));
                ui->listWidgetUserList->item(i)->setForeground(QBrush(QColor("Black")));
            }
    ...
    if (IPMSG_BR_ENTRY == command || IPMSG_ANSENTRY == command) {
        ...
        if (i != mUsers.count()) {
            ...
            connect(user->chatForm,
                    ...
                    this,
                    SLOT(qIpMsgChatSent(QString, QHostAddress)));
            connect(user->chatForm,
                    SIGNAL(acceptFile(fileEntryT *)),
                    this,
                    SLOT(qIpMsgAcceptFile(fileEntryT *)));
            connect(user->chatForm,
                    SIGNAL(rejectFile(fileEntryT *)),
                    this,
                    SLOT(qIpMsgRejectFile(fileEntryT *)));
            connect(user->chatForm,
                    SIGNAL(recvfinish(quint32)),
                    this,
                    SLOT(qIpMsgRecvFileFinish(quint32)));
            connect(user->chatForm,
                    SIGNAL(recverror(quint32)),
                    this,
                    SLOT(qIpMsgRecvFileError(quint32)));
            ...
            ui->listWidgetUserList->addItem(
                new QListWidgetItem(QIcon(":/userIcon/chat.png"),
                                    toUnicode(mUsers.at(i)->userNickName) + "("
                                        + toUnicode(mUsers.at(i)->userHostName) + ")" + "\n"
                                        + mUsers.at(i)->userHostAddr.toString()));
            qIpMsgStatus(tr("Found") + " " + QString::number(mUsers.count(), 10) + " "
                         + tr("users"));
            //mUsers.at(i)->dumpUserInfo();
            //qDebug()<<"Pkt Seq:"<<mSelf.userPktSeq;
        } else {
            ui->listWidgetUserList->item(i)->setText(
                toUnicode(mUsers.at(i)->userNickName) + "(" + toUnicode(mUsers.at(i)->userHostName)
                + ")" + "\n" + mUsers.at(i)->userHostAddr.toString());
        }
        ui->listWidgetUserList->item(i)->setForeground(QBrush(QColor("Black")));
        ...
        if (IPMSG_FILEATTACHOPT & commandOpt) {
            ...
            qDebug() << "Receive Msg from"
                     << "User:" << QString::number(i, 10) << values.at(3)
                     << ":" + chatData.split(0).at(0);
        }
        ...
        if (reqFileId == mSelf.fileQueue.at(i)->fileId && ip == mSelf.fileQueue.at(i)->fileHost
            ...
            mSelf.fileQueue.at(i)->fileTranStatus = FILE_TRANS_STATUS_RUN;
            {
                ...
                connect(trans,
                        ...
                        this,
                        SLOT(qIpMsgFileTransProgress(quint32, int)));
                connect(trans,
                        SIGNAL(fileTransFinished(quint32)),
                        this,
                        SLOT(qIpMsgFileTransFinished(quint32)));
                connect(trans,
                        SIGNAL(fileTransError(quint32, int)),
                        this,
                        SLOT(qIpMsgFileTransError(quint32, int)));
        ...
        if (mUsers.at(i)->chatForm != nullptr) {
            for (int j = 0; j < mUsers.at(i)->chatForm->fileList.count(); j++) {
                if (mUsers.at(i)->chatForm->fileList.at(j)->fileId == fileId
                    ...
                    mUsers.at(i)->appendChatHistory(
                        tr("Send") + " \""
                        + mUsers.at(i)->chatForm->fileList.at(j)->info.absoluteFilePath + "\" "
                        + tr("error"));
                    mUsers.at(i)->appendChatHistory("\n");
                    mUsers.at(i)->updateChatFormHistory();
...
void QIPMSG::qIpMsgRecvFileFinish(quint32 fileId)
{
    ...
    if (form != nullptr) {
        ...
        for (i = 0; i < form->fileList.count(); i++) {
            if (form->fileList.at(i)->fileId == fileId && form->fileList.at(i)->fileOut == false
                ...
                user->appendChatHistory(tr("Receive") + " \""
                                        + form->fileList.at(i)->info.absoluteFilePath + "\" "
                                        + tr("finished"));
                user->appendChatHistory("\n");
